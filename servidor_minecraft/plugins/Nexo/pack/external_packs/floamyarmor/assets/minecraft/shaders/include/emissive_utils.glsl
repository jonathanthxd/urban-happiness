...
vec4 make_emissive(vec4 inputColor, vec4 lightColor, vec4 faceLightColor, int inputAlpha) {
    ...
    if (inputAlpha == 254) return inputColor; // Checks for alpha 252 and just returns the input color if it is. Used in the example pack for redstone ore and the zombie's eyes.
    if (inputAlpha == 128) return inputColor; // Checks for alpha 128 and just returns the input color if it is. Used in the example pack for redstone ore and the zombie's eyes.
...
vec4 make_emissive(vec4 inputColor, vec4 lightColor, vec4 maxLightColor, float vertexDistance, float inputAlpha) {
	...
	else if (check_alpha(inputAlpha, 254.0)) return inputColor; // You can copy & this line and change the function to add a new emissive type. Used in the example pack for lime concrete. 
	else if (check_alpha(inputAlpha, 128.0)) return inputColor; // You can copy & this line and change the function to add a new emissive type. Used in the example pack for lime concrete. 
